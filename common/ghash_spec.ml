...
(*                                                                           *)
(* NEXT STEPS:                                                               *)
(*   1. Generalize to 4-block (GHASH_POLYVAL_ACC_4) for Loop4x              *)
(*   2. Define batched GHASH matching assembly structure                      *)
(*   3. Start ARM simulation of gcm_ghash_v8                                 *)
(* ========================================================================= *)
(*                                                                           *)
(* PROVEN above:                                                             *)
(*   BOOL_POLY_MUL, BOOL_POLY_ZERO, INTEGRAL_DOMAIN_BOOL_POLY,              *)
(*   POLYVAL_POLY_NONZERO, POLYVAL_DIVIDES_LOW_DEG, MOD_POLYVAL_WORD_EQ,     *)
(*   polyval_dot (def), POLYVAL_DOT_CORRECT,                                 *)
(*   ghash_polyval_acc (def), GHASH_ACC_APPEND, GHASH_ACC_STEP_CONG          *)
(*                                                                           *)
(* NEXT STEPS:                                                               *)
(*   1. Prove RING_IRREDUCIBLE_POLYVAL_POLYNOMIAL (Q(x) irreducible)         *)
(*      - Follow same Rabin test as RING_IRREDUCBLE_GHASH_POLYNOMIAL         *)
(*        in common/ghash.ml lines 594-650                                   *)
(*      - Need X_POWPOW analogs for Q(x): x^(2^128) ≡ x and                *)
(*        x^(2^64) not ≡ x (mod Q(x))                                       *)
(*      - Then x^128 is invertible in GF(2^128) = GF(2)[x]/Q(x)            *)
(*   2. Prove n-block Horner unrolling (general, by induction):              *)
(*      ghash_polyval_acc H acc [X1;...;Xn]                                  *)
(*      = prop3(pmul(acc⊕X1, Kn) ⊕ ... ⊕ pmul(Xn, K1))                     *)
(*      where K_k are Htable entries with baked-in reductions                *)
(*   3. Target file: arm/aes-gcm/aesv8-gcm-armv8-unroll8.S (8-block)        *)
(*                                                                           *)
(* KEY INSIGHT: Htable stores K_k = H̄^k * x^{-(k-1)*128}, not raw H̄^k.    *)
(* This makes batched = iterative. One inductive proof covers 2/4/8-block.   *)
(*                                                                           *)
(* DEAD ENDS: None significant. MESON_TAC times out on biconditionals        *)
(* involving poly_0/ring_0 — use explicit EQ_TAC instead.                    *)
(* ========================================================================= *)
